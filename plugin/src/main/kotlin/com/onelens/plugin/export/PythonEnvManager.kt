...
object PythonEnvManager {
    ...
    private fun installOneLens(sourcePath: String): Boolean {
        ...
        // If source path provided (dev mode), install from local path
        val packageSpec = if (sourcePath.isNotEmpty() && File(sourcePath).exists()) {
            sourcePath
        } else {
            "onelens"  // Install from PyPI
        }

        // Try uv pip install first (10x faster)
        if (uvBin != null) {
            val success = runCommand(
                listOf(uvBin, "pip", "install", packageSpec, "--python", VENV_DIR.resolve("bin").resolve("python").toString()),
                ONELENS_HOME.toFile()
            )
            if (success) {
                LOG.info("Installed onelens with uv")
                return true
            }
        }
        ...
        if (runCommand(listOf(pip, "install", packageSpec), ONELENS_HOME.toFile())) {
            LOG.info("Installed onelens with pip")
            return true
        }
