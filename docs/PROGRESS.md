...
| E5.3a | Stage 3a — `EnumLoader` owns full + delta enum constants (`:Field:EnumConstant` dual-label + HAS_ENUM_CONSTANT). The subsystem delta split into two nodes before this session's fix. Cascade-delete in core untouched. Delegated Sonnet agent; 19/19 green. 5 subsystems single-sourced. | ✅ | `importer/loaders/enums.py` |
| E5.3b | Stage 3b — `TypeFlowLoader`: shared `_build` for RETURNS/THROWS/HAS_PARAMETER edge lists; writes stay path-specific (full MATCH, delta MERGE-inline). Delegated Sonnet agent; 19/19 green. | ✅ | `importer/loaders/type_flow.py` |
| E5.3c | Stage 3c — `DataFlowLoader`: shared `_build` for READS/WRITES_FIELD/INSTANTIATES; path-specific writes. Delegated Sonnet agent; 19/19 green. **7 subsystems single-sourced; loader.py 1763→1100, delta_loader.py 905→432.** | ✅ | `importer/loaders/data_flow.py` |
| E5.3d | Optional — `LOADERS` registry so `load_full`/`apply_delta` iterate a list (overlay subsystems). Cosmetic now that all subsystems are single-sourced loaders; core stays orchestration backbone. | ⬜ | design §3 |
