...
| E5.3b | Remaining — type-flow (RETURNS/THROWS/HAS_PARAMETER) + data-flow (READS/WRITES_FIELD/INSTANTIATES) loaders + `LOADERS` registry. Involves stub-mechanism unification (full pre-collects, delta MERGEs inline) + call-ordering — higher-care; resume with fresh context. | ⬜ | design `multi-language-architecture.md` §3 |
