...
| H3 | Verified remaining `example`/`example` occurrences are all in gitignored local-dev files (`python/trial_*.py`, `python/modal_index*.py`, `python/benchmarks/*.yaml`, `.claude/settings.local.json`) — not in the shipped artifact. Any attempt to commit them would fail the hook. | ✅ | — |
