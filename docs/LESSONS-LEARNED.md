...
- **Inter-project dependency caveat**: Sibling repos with dependencies on the primary project's artifacts (e.g. `com.flotomate.common`) need `mvn install -DskipTests` on the primary first. Without it, Maven creates the sibling modules but can't resolve their dependencies → IntelliJ can't create proper source roots → those classes never enter the stub index.
