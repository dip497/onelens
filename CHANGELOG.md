...
- `Field` ∪ `JpaColumn` → one node carrying both labels. Was emitting a
  ...
  already wrote (~5.8k duplicates on example).
...
- **Benchmark** (example, 548 MB export, 10k classes, 80k methods, 630k call
...
- `@SpringBootTest` on a superclass up the chain (example's
...
- `loader.py`, `delta_loader.py`, `code_miner.py` swap stdlib `json.load` →
  ...
  faster on 120 MB synthetic exports; ~2-3 s saved per sync on example's
...
- Enables precise impact queries: "rename `Request.priorityId` — what
  ...
  popularity rankings.
- 77.9% alias-resolution rate on example's 201 customer queries (9807
...
- `miners/flyway_detector.py` — auto-scans `application*.{properties,yml}` for
  ...
  example's `classpath:db/migration/tenants` which uses a custom loader).
...
- `plugin/.../framework/workspace/WorkspaceLoader.kt` — parses
  ...
  roots resolving against the config's directory, so the example
  pattern (`- path: ../sibling_plugins`) just works. Absent config
